...
exports.createAgentConstitution = createAgentConstitution;
function createAgentConstitution(input, passportId) {
    ...
    const prohibitedList = input.prohibitedActions.length > 0 ? input.prohibitedActions.join(', ') : 'none declared';
    const escalationText = input.escalationRules.length > 0
        ? input.escalationRules
            .map((r) => `Trigger: ${r.trigger} → Action: ${r.action}${r.escalateTo ? ` (escalate to: ${r.escalateTo})` : ''}`)
            .join('; ')
    ...
    const articles = [
        ...
        },
        {
            number: 'VII',
            title: 'Auditability',
            content: 'All actions taken by this agent must be auditable. The agent must maintain a complete audit trail of all governance decisions, data access events, and tool invocations.',
        },
        {
            number: 'VIII',
            title: 'Suspension, Revocation, and Termination',
            content: "This agent's passport may be suspended, revoked, or expired according to the AOC governance lifecycle. Removing the passport does not destroy the agent, but it destroys the agent's AOC-governed verification status. In governed runtimes, missing or invalid passports must cause governed execution to fail.",
        },
    ];
    return {
