 ...
 * proper normalization, replay, and Evidence Bundle support. Table/column
 * names (including `RuntimeEvents`/`RuntimeVersions`) are unchanged from
 * PR-002 -- this iteration renames TypeScript types, not the schema.
 */
export async function createSqliteGovernanceStore(dbPath: string): Promise<GovernanceStore> {
  ...
  const insertVersion = db.prepare(
    `INSERT INTO RuntimeVersions (boot_id, runtime_version, kernel_version, recorded_at) VALUES (@bootId, @enterpriseVersion, @kernelVersion, @recordedAt)`,
  );
  ...
  return {
    ...
    async appendEnterpriseEvent(record: EnterpriseEventRecord) {
      insertEvent.run({
    ...
    async getLatestEnterpriseVersion() {
      ...
      return row === undefined
        ? undefined
        : { bootId: row.boot_id, enterpriseVersion: row.runtime_version, kernelVersion: row.kernel_version, recordedAt: row.recorded_at };
    },
...
function rowToEventRecord(row: SqliteEventRow): EnterpriseEventRecord {
  return {
