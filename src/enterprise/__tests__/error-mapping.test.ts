...
describe('EnterpriseHttpErrors', () => {
  it('produces the mission-specified status code and machine-readable code for each error kind', () => {
    const cases: readonly [() => { httpStatus: number; code: string }, number, string][] = [
      [() => EnterpriseHttpErrors.invalidRequest('x'), 400, 'INVALID_REQUEST'],
      [() => EnterpriseHttpErrors.authenticationFailed(), 401, 'AUTHENTICATION_FAILED'],
      [() => EnterpriseHttpErrors.authorizationFailed(), 403, 'AUTHORIZATION_FAILED'],
      [() => EnterpriseHttpErrors.concurrencyConflict('x'), 409, 'CONCURRENCY_CONFLICT'],
      [() => EnterpriseHttpErrors.infrastructureFailure(), 500, 'INFRASTRUCTURE_FAILURE'],
      [() => EnterpriseHttpErrors.providerUnavailable(), 503, 'PROVIDER_UNAVAILABLE'],
    ];
  ...
  it('carries validation details through to the response shape', () => {
    const error = EnterpriseHttpErrors.invalidRequest('bad request', ['field a is required']);
