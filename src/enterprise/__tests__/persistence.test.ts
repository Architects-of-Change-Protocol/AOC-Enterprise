...
for (const variant of STORE_VARIANTS) {
  describe(`GovernanceStore (${variant.name})`, () => {
    ...
    it('appends and filters Enterprise events by requestId and correlationId', async () => {
      const store = await variant.build();
    ...
    it('records and retrieves the latest EnterpriseVersion', async () => {
      const store = await variant.build();
