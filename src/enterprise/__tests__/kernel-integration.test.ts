...
describe('Soberanía Enterprise Host / Kernel parity', () => {
  ...
  it('never calls kernel.enforce() -- evaluate() only, per the mission (no execution, no Passport mutation)', () => {
    ...
    const orchestrationSource = readFileSync('src/enterprise/orchestration/evaluate-governance-request.ts', 'utf8');
  ...
  it('surfaces a real recognition denial exactly as the Kernel reports it, with a full persisted trace', async () => {
    const enterprise = await createEnterprise({ kernelProviders: buildTestKernelProviders() });
    const outcome = await enterprise.evaluate(buildDeniedRequestBody({ requestId: 'req-parity-denied-1' }));
