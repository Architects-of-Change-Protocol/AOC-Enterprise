import { RUNTIME_PERSISTENCE_SERIALIZATION_VERSION, RUNTIME_PERSISTENCE_SNAPSHOT_VERSION, type RuntimePersistenceEnvelope, type RuntimePersistenceValidationReport } from './runtime-persistence-types';
export function validateRuntimePersistenceEnvelope(envelope: unknown): RuntimePersistenceValidationReport {
  ...
  const lifecycle = e?.snapshot?.lifecycleMarkers ?? []; for (let i = 1; i < lifecycle.length; i += 1) if (lifecycle[i].sequence <= lifecycle[i - 1].sequence) errors.push('lifecycle_order_invalid');
  ...
  else if (Date.parse(e.persistenceTimestamp) < Date.parse(e.snapshot.operational.metadata.createdAt)) warnings.push('persistence_timestamp_before_runtime_creation');
