...
describe('Concurrent governance evaluation', () => {
  it('handles many concurrent requests with distinct requestIds without cross-talk', async () => {
    ...
    const outcomes = await Promise.all(
      Array.from({ length: 20 }, (_, i) => host.handleGovernanceEvaluate({ rawBody: buildAllowedRequestBody({ requestId: `req-concurrent-${i}` }) })),
    );
    ...
    for (let i = 0; i < 20; i += 1) {
      assert.ok(await host.store.getRequestById(`req-concurrent-${i}`), `req-concurrent-${i} must be durably persisted`);
    }
  ...
  it('resolves a concurrent burst of the same requestId+payload to exactly one stored decision, everyone else replaying it', async () => {
    ...
    const outcomes = await Promise.all(Array.from({ length: 10 }, () => host.handleGovernanceEvaluate({ rawBody: body })));
  ...
  it('resolves a concurrent burst of the same requestId with conflicting payloads to exactly one winner and 409s the rest', async () => {
    ...
    const outcomes = await Promise.allSettled(
      Array.from({ length: 5 }, (_, i) =>
        host.handleGovernanceEvaluate({
          rawBody: buildAllowedRequestBody({ requestId: 'req-concurrent-conflict', context: { passportId: 'passport-pmfreak', capabilityTokenId: 'cap-pmfreak-drafting', evidence: [], tag: i } }),
        }),
      ),
    );
