...
describe('validateGovernanceEvaluateRequestBody', () => {
  ...
  it('rejects a non-object body', () => {
    assert.throws(() => validateGovernanceEvaluateRequestBody('not an object'), RuntimeHttpError);
    assert.throws(() => validateGovernanceEvaluateRequestBody(null), RuntimeHttpError);
    assert.throws(() => validateGovernanceEvaluateRequestBody([1, 2, 3]), RuntimeHttpError);
  });
  ...
  it('collects every violation into RuntimeHttpError.details', () => {
    try {
...
describe('toKernelEvaluationRequest', () => {
  it('generates requestId/requestedAt when the caller omits them', () => {
    ...
    assert.equal(request.requestId, 'runtime-request-generated');
...
describe('toKernelEvaluationOptions', () => {
  it('defaults traceLevel to the Runtime configuration when the caller omits it', () => {
