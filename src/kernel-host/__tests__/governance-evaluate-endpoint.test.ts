...
import { createRuntimeHostServer, type RuntimeHostServer } from '../host/runtime-host-server.js';
...
const startedServers: RuntimeHostServer[] = [];

async function startTestServer(overrides: Parameters<typeof createRuntimeHostServer>[0] = {}): Promise<{ server: RuntimeHostServer; baseUrl: string }> {
  const configuration =
    overrides.configuration ??
    loadRuntimeConfiguration({
      AOC_RUNTIME_HTTP_PORT: '0',
      AOC_RUNTIME_HTTP_HOST: '127.0.0.1',
      AOC_RUNTIME_EVENTS_ENABLED: 'true',
    });
  const server = await createRuntimeHostServer({ kernelProviders: buildTestKernelProviders(), ...overrides, configuration });
...
describe('POST /api/governance/evaluate', () => {
  ...
  it('enforces authentication when configured, and 403s a credential scoped to a different organization', async () => {
    const configuration = loadRuntimeConfiguration({
      AOC_RUNTIME_HTTP_PORT: '0',
      AOC_RUNTIME_HTTP_HOST: '127.0.0.1',
      AOC_RUNTIME_REQUIRE_AUTH: 'true',
      AOC_RUNTIME_API_KEYS: 'key-open,key-acme:org-acme',
    });
...
describe('GET /health', () => {
  it('reports runtime/kernel version, database connectivity, and provider status with no sensitive fields', async () => {
    const started = await startTestServer();
    ...
    assert.equal(body.providers.recognitionProvider, true);
