...
import { RuntimeHttpErrors } from './runtime-http-errors.js';

/** Wire shape for `POST /api/governance/evaluate`. A structural subset of `KernelEvaluationRequest` -- the Runtime never adds fields the Kernel doesn't already define, per "No Runtime module may manipulate governance semantics." */
...
/** Wire shape returned from `POST /api/governance/evaluate` -- exactly the mission's Response Contract, plus `requestId`/`correlationId` for client-side correlation (neither is internal Runtime state). */
...
/**
 * The Runtime's own request validation -- HTTP/JSON shape only. It
...
export function validateGovernanceEvaluateRequestBody(body: unknown): GovernanceEvaluateRequestBody {
  ...
  if (!isPlainObject(body)) {
    throw RuntimeHttpErrors.invalidRequest('Request body must be a JSON object.');
  }
  ...
  if (violations.length > 0) {
    throw RuntimeHttpErrors.invalidRequest('The governance evaluation request failed validation.', violations);
  }
...
export function toKernelEvaluationRequest(body: GovernanceEvaluateRequestBody, clock: KernelClock, idGenerator: KernelIdGenerator): KernelEvaluationRequest {
  return {
    requestId: body.requestId ?? idGenerator.nextId('runtime-request'),
    actor: body.actor,
    action: body.action,
 ...
 * mission's Runtime Error Model ("422 Governance denial ... is NOT an
