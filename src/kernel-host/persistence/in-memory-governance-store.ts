 ...
 * this is what the Runtime Host boots with out of the box and what its own
 * test suite exercises against, in line with the mission's "keep schema
 * minimal, avoid premature normalization" guidance for a v1.
 */
export function createInMemoryGovernanceStore(): GovernanceStore {
  ...
  return {
    ...
    async appendRuntimeEvent(record) {
      events.push(record);
    },
    ...
    async recordRuntimeVersion(record) {
